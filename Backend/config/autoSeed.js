/**
 * @file autoSeed.js
 * @description Automatic database seeder. Seeds earthquake data on server startup if collection is empty.
 * @module config/autoSeed
 */
...
const autoSeed = async () => {
  try {
    console.log('🔄 Checking database for default admin and records...');
    
    // 1. Seed Admin if needed
    const email = 'arpan@example.com';
    let user = await User.findOne({ email });
    if (!user) {
      console.log('👤 Creating default admin user (arpan@example.com / Password123)...');
      await User.create({
        name: 'Arpan Shah',
        email: email,
      ...
      console.log('👤 Updating existing user to admin & verified status...');
      user.role = 'admin';
      user.isVerified = true;
      await user.save();
      console.log('✅ Admin user status updated.');
    } else {
      console.log('✅ Default admin user is configured.');
    }

    // 2. Seed Earthquake records if count is low
    const existing = await Earthquake.countDocuments({ isDeleted: { $ne: true } });
    console.log(`📊 Existing earthquake records in DB: ${existing}`);

    if (existing >= 2000) {
      console.log('✅ Sufficient earthquake records already exist. Skipping seeder.');
      return;
    }

    console.log('🌱 Database has fewer than 2000 records. Generating 5000 realistic records...');
    ...
    const finalCount = await Earthquake.countDocuments({ isDeleted: { $ne: true } });
    console.log(`\n🎉 Success! Seeding finished. Current records in DB: ${finalCount}`);
  } catch (error) {
